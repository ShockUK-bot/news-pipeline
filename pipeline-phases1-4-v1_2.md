# News Pipeline — Phases 1–4 (v1.2)

**Consolidated implementation document.** Replaces `pipeline-phases1-4-v1_1.md`
in project knowledge. Repo: `ShockUK-bot/news-pipeline` (private). Version 0.4.6.

## v1.2 changelog — CIK→ticker mapping + test-DB guard (v0.4.6)

1. **CIK→ticker mapping** (`src/c1_ingestion/cik_map.py`): SEC's
   company_tickers.json cached at edgar.cik_map_path (default
   /opt/pipeline/var/cik_map.json), refreshed when older than
   cik_map_refresh_hours (24); refresh failures degrade to the stale cache.
   The EDGAR poller stamps `symbols` deterministically from entity CIKs —
   A1 no longer infers tickers for EDGAR items; the position-touching
   fast path works on filings from ingestion onward.
2. **skip_unmapped** (default true): whitelisted filings whose entities map
   to no listed ticker (trusts, funds, private filers) are archived without
   triage. Fail-safe: ignored when the map is empty/unfetchable, so a
   failed SEC fetch can never black-hole the feed — unmapped items then
   flow to A1 whose ticker inference remains the fallback (baseline §4 A1).
3. **tests/conftest.py guard** (from the 2026-07-14 production-DB incident):
   pytest refuses to run unless PIPELINE_DSN names a *_test database —
   integration fixtures TRUNCATE. Canonical test invocation:
   `PIPELINE_DSN=...trading_test EMBEDDER=hash QDRANT_PATH=/tmp/qdrant-test pytest tests/ -q`
   Suite: 200 tests green (189 + 11 CIK/guard).
