...
A locally hosted, news and sentiment driven, multi-agent trading pipeline for US equities. 13 agents (A1 triage through A13 operator chat) plus supporting components C1 through C10, built in versioned phases. Currently at tag `v0.25.2` (verify with `git describe --tags`).
...
Long running components: `c1-ingestion c2-dedup c3-gate c4-exec c6-dashboard c8-regime c10-scanner`. `c12-burst` (v0.15.0 to v0.24) is RETIRED since v0.25.0 (2026-09-22): stopped and disabled, unit file kept, `journal.burst_events` kept as research history, `src/c12_burst/book.py` and `detect.py` kept as a library for the scanner (see `docs/claude_c12-retirement-and-reuse-2026-09-22.md`). Do not start it.
Scheduled components (oneshot plus `.timer`): `c5-mailer c7-watchdog`. Emails (v0.22.0): two a day, the 06:35 CT morning briefing (A8) and the 21:20 CT evening digest, plus Saturday's A9 review; all HTML with a text fallback (`journal.outbox.html`, `common/mailkit.py`). Watchdog warnings alert only on the second consecutive pass and never send a recovery email; criticals alert at once and recover with an email.
Support timers (oneshot plus `.timer`): `backup-offsite earnings-calendar evening-digest journal-extract llama-heavy-guard macro-fetch pipeline-backup pipeline-nav-snapshot queue-prune sector-map thesis-entry` (v0.25.x: `backup-offsite` 02:50 copies the dump to Google Drive via `~/.local/bin/rclone`, `journal-extract` 03:05 force pushes a compact history to the GitHub branch `journal-extract`, `llama-heavy-guard` 08:15 stops a heavy slot left running; see `docs/claude_backup-offsite-2026-09-22.md`) (`evening-digest` since v0.22.0: 21:20 CT weekdays, the ONE evening email composed from what A7, A6, A5 and C11 journaled; those four no longer email on their own, `email: false` in their yaml; heartbeat `digest`) (`sector-map` since v0.19.0: 04:40 CT, ticker to sector from SEC SIC codes into `journal.sectors`, heartbeat `sectors`). Vector store: `qdrant`.
